...
UEdGraphPin* UComputeShaderGraphNode_Base::GetOutputPin()
{
...
void UComputeShaderGraphNode_Base::GetInputPins(TArray<class UEdGraphPin*>& OutInputPins)
{
	OutInputPins.Empty();
...
UEdGraphPin* UComputeShaderGraphNode_Base::GetInputPin(int32 InputIndex)
{
...
int32 UComputeShaderGraphNode_Base::GetInputCount() const
{
	int32 InputCount = 0;
...
void UComputeShaderGraphNode_Base::InsertNewNode(UEdGraphPin* FromPin, UEdGraphPin* NewLinkPin, TSet<UEdGraphNode*>& OutNodeList)
{
	const UComputeShaderGraphSchema* Schema = CastChecked<UComputeShaderGraphSchema>(GetSchema());
...
void UComputeShaderGraphNode_Base::AllocateDefaultPins()
{
	check(Pins.Num() == 0);
	...
	if (!IsRootNode())
	{
		CreatePin(EGPD_Output, TEXT("ComputeShaderNode"), TEXT("Output"));
	}
}

void UComputeShaderGraphNode_Base::ReconstructNode()
{
...
void UComputeShaderGraphNode_Base::AutowireNewNode(UEdGraphPin* FromPin)
{
	if (FromPin != NULL)
	{
		const UComputeShaderGraphSchema* Schema = CastChecked<UComputeShaderGraphSchema>(GetSchema());
...
bool UComputeShaderGraphNode_Base::CanCreateUnderSpecifiedSchema(const UEdGraphSchema* Schema) const
{
	return Schema->IsA(UComputeShaderGraphSchema::StaticClass());
}

FString UComputeShaderGraphNode_Base::GetDocumentationLink() const
{
	// FIXME
	// TODO
